...
| optionSearchLabelProp | which property of option data is used to populate the search value once said option is selected (only works in combobox mode) | string | 'value' |
| allowSearchLabelOnly | ensures that the search value only matches a selected option's search label when the search field no longer has focus (only works in combobox mode) | bool | false |
